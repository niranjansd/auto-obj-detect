(function (global, factory) {
  ...
  ***************************************************************************** */
  ...
  function drawAllDetections(canvasArg, detections) {
    for (var i = 0; i < detections.length; i++) {
        drawDetections(canvasArg[i], detections[i]);
    }
  }
  ...
  function ImageSlider() {
    ...
    //image url
    $("#sliderimage").attr("src", imgs[i].src);
    $("#slideroverlay").attr("src", overlays[i].src);
...
function getMediaDimensions(input) {
  ...
  function matchAllDimensions(input, reference, useMediaDimensions) {
    var htwt = [];
    for (i=0; i<reference.length; i++){
        htwt.push(matchDimensions(input[i], reference[i], useMediaDimensions))
        // var _a = useMediaDimensions
        //     ? getMediaDimensions(reference[i])
        //     : reference[i], width = _a.width, height = _a.height;
        // input[i].width = width;
        // input[i].height = height;
        // htwt.push({ width: width, height: height });
    }
    return htwt;
    }
  ...
  var index$1 = /*#__PURE__*/Object.freeze({
    ...
    drawAllDetections: drawAllDetections,
  ...
  function resizeAllResults(results, dimensions) {
    for (var i = 0; i < results.length; i++) {
      var _a = new Dimensions(dimensions[i].width, dimensions[i].height), width = _a.width, height = _a.height;
      if (width <= 0 || height <= 0) {
          throw new Error("resizeResults - invalid dimensions: " + JSON.stringify({ width: width, height: height }));
      }
      if (Array.isArray(results[i])) {
          results[i].map(function (obj) { return resizeResults(obj, { width: width, height: height }); });
      }
      if (isWithFaceLandmarks(results[i])) {
          var resizedDetection = results[i].detection.forSize(width, height);
          var resizedLandmarks = results[i].unshiftedLandmarks.forSize(resizedDetection.box.width, resizedDetection.box.height);
          extendWithFaceLandmarks(extendWithFaceDetection(results[i], resizedDetection), resizedLandmarks);
      }
      if (isWithFaceDetection(results[i])) {
        extendWithFaceDetection(results[i], results[i].detection.forSize(width, height));
      }
      if (results[i] instanceof FaceLandmarks || results[i] instanceof FaceDetection) {
          results[i].forSize(width, height);
      }
    }
    return results;
  }
  ...
  exports.matchAllDimensions = matchAllDimensions;
  ...
  exports.resizeAllResults = resizeAllResults;
